import { RequestChangePointName, RequestProblemName, SubmitChangePointActionName, SubmitProblemActionName } from "@/constant";
import axiosInstance from "@/lib/axios";
import { UserStore, _5M1ESettingStore } from "@/store";
import { RequestConfigStore } from "@/store/request-config.store";
import { IRequestForm, ISubmitRequestFormParam, IUpdateRequestFormParam } from "@/types/request-form.type";
import { _5M1ERequest } from "@/types/request.type";

async function draftRequestForm(form: IRequestForm, problemRequestId: string | null, startActionName: string, requestProcessName: string): Promise<any> {
  const { user } = UserStore.getState()
  const { getRequestProcessByName } = _5M1ESettingStore.getState()
  const { getActionByName, getTransitionByActionId } = RequestConfigStore.getState()
...
export async function updateRequestForm(transitionId: string, form: _5M1ERequest, emailList: string[], actionNote: string, supporterList: string[], confirmationList: string[]): Promise<any> {
  const { user } = UserStore.getState()
  ...
  const body: IUpdateRequestFormParam = {
    ...
    request_process_name: requestProcessDict[form.request_process_id].request_process_short_name,
    request_process_id: form.request_process_id,
    action_user_uuid: user?.user_uuid ?? '',
    line_id: form.request_data_value.lineId,
    user_uuid: form.user_uuid,
    email_list: emailList.length === 0 ? supporterList.length === 0 ? confirmationList : supporterList : emailList,
...
export async function updateRequestFormValue(request: _5M1ERequest, form: IRequestForm): Promise<any> {
  const { requestProcessDict } = _5M1ESettingStore.getState()

  const body: IUpdateRequestFormParam = {
    request_id: request.request_id,
    request_process_name: requestProcessDict[request.request_process_id].request_process_short_name,
...
export async function draftRequestProblemForm(form: IRequestForm): Promise<any> {
...
export async function draftRequestChangePointForm(form: IRequestForm, problemRequestId: string | null): Promise<any> {
  return draftRequestForm(
    form,
    problemRequestId,
