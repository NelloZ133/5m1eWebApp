...
export const ActionPanel: FC<IProps> = ({ request }: IProps) => {
  ...
  const [selectedTransition, setSelectedTransition] =
    useState<RequestTransition | null>(null);
  ...
  const handleActionModalFinish = async (form: any) => {
    ...
    if (transitionId === undefined) {
      message.error(
        `Not found transition with name: ${selectedTransition?.description}`
      );
  ...
  const handleClickAction = async (transition: RequestTransition) => {
    ...
    if (shouldPromptModal(transition)) {
      ...
      if (transitionId === undefined) {
        message.error(
          `Not found transition with name: ${transition.description}`
        );
        return setIsLoading(false);
      }
      try {
        await updateRequestForm(`${transitionId}`, request, [], "", [], []);
        await getAllRequest();
      } catch (error) {
        console.error(error);
      } finally {
    ...
    <>
      <ActionModal
        ...
        showSelectConfirmation={
          selectedTransition
            ? isShowSelectConfirmation(selectedTransition)
            : false
        }
        showEmailSelect={
          selectedTransition ? isShowMailingList(selectedTransition) : false
        }
        showActionInput={
          selectedTransition ? isShowActionNote(selectedTransition) : false
        }
        showSelectSupporter={
          selectedTransition ? isShowSelectSupporter(selectedTransition) : false
        }
        onFinish={handleActionModalFinish}
        onCancel={() => setActionModal(false)}
      />
      {getFilteredTransitionByCurrentStateId(
        +request.actionList[request.actionList.length - 1].current_state_id
      )
        .filter((transition) =>
      ...
      {isRequestProblemFinished(
        transition?.[
          request.actionList[request.actionList.length - 1].transition_id
        ]
      ) ? (
        <Button onClick={handleSubmitChangePoint}>Submit Change Point</Button>
      ) : null}
    </>
  );
};
