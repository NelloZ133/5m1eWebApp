...
export const ActionModal: FC<IProps> = ({ title, isLoading, showSelectSupporter, showEmailSelect, showActionInput, showSelectConfirmation, visible, onFinish, onCancel }: IProps) => {
  const [form] = useForm()
  const { availableEmailForSubmitList, availableSupporterList } = _5M1ESettingStore()
        ...
        <Form
          ...
          {showEmailSelect ?
            <Form.Item
              label="Mailing List"
              name="mailList"
              rules={[{ required: true, message: 'Please select notify people' }]}>
              <Select
          ...
          {showActionInput ?
            <Form.Item
              label="Action Note"
              name="actionNote"
              rules={[{ required: true, message: 'Please input action note' }]}>
          ...
          {showSelectConfirmation ? <Form.Item
            label="Confirmation"
            name="confirmationList"
            rules={[{ required: true, message: 'Please select confirmation' }]}>
            <Select
              items={availableSupporterList()}
              labelKey="email"
              valueKey="email"
