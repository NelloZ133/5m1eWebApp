...
const FileUpload: FC<IProps> = ({ listType, fileList, validator, onChange, ...props }: IProps) => {
  const onFileUploaded = (file: UploadChangeParam<UploadFile>) => {
    const attachmentList = file?.fileList ?? []
    const validatedFileList = attachmentList.filter((v: UploadFile) => validator !== undefined ? validator(v) : true)
