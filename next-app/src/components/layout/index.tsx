...
const Container: FC<IProps> = ({ title, backable, backRoute, hideAuthSection, children }: IProps) => {
  const { isLoading } = LayoutStore()
  return (
    <Spin spinning={isLoading} size="large" style={{ top: '50%', transform: 'translateY(-50%)' }}>
      <Header title={title} backable={backable} hideAuthSection={hideAuthSection} backRoute={backRoute}/>
