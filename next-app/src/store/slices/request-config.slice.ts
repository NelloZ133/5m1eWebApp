...
export const RequestConfigSlice: StateCreator<IRequestConfigState> = (set, get) => ({
  action: {},
  activity: {},
  state: {},
  transition: {},
  ...
  getActionByName(name) {
    return get().actionList().find(action => action.name === name)
  },

  getTransitionByActionId(id) {
    return get().transitionList().find(transition => +transition.action_id === +id)
  },

  getTransitionByName(name) {
    return get().transitionList().find(transition => transition.description === name)
  },
  
  getFilteredTransitionByCurrentStateId(currentStateId) {
    return get().transitionList().filter(transition => +transition.current_state_id === currentStateId)
  }
})
