...
export const _5M1EReportSettingSlice: StateCreator<I5M1EReportSettingState> = (set, get) => ({
  requestProcessDict: {},
  itemDetailDict: {},
  lineDict: {},
  linePartDict: {},
  ...
  setReportSetting(response) {
    set({
      ...
      itemDetailDict: Object.keys(response.item_details).map(v => +v).reduce<Record<number, ItemDetail[]>>((prev, cur) => {
        prev[cur] = response.item_details[cur].data
        return prev
      }, {}),
      lineDict: response.lines,
      linePartDict: response.lines_parts,
      listItemProblemDict: Object.keys(response.list_items_problem).reduce<Record<string, ListItem[]>>((prev, cur) => {
        prev[cur] = response.list_items_problem[cur].data
        return prev
      }, {}),
      listItemChangePointDict: Object.keys(response.list_items_changepoint).reduce<Record<string, ListItem[]>>((prev, cur) => {
  ...
  getRequestProcessByName(name) {
    return get().requestProcessList().find(reqProcess => reqProcess.request_process_name === name)
  },

  getItemDetailById(itemDetailId) {
    return get().itemDetailList().find(itemDetail => itemDetail.item_detail_id === itemDetailId)
  },

  getProblemListItemById(listItemId) {
    return get().listItemProblemList().find(listItem => listItem.list_item_id === listItemId)
  },
  
  getChangePointListItemById(listItemId) {
    return get().listItemChangePointList().find(listItem => listItem.list_item_id === listItemId)
  }
})
