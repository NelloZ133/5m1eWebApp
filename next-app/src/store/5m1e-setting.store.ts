import { fetch5M1EReportSetting, fetch5M1ESystemSetting, fetchRequestConfig } from "@/actions";
...
export const _5M1ESettingStore = create<I5M1ESettingState & I5M1EProblemPermissionState & I5M1EReportSettingState & I5M1ESystemSettingState>((...args) => ({
  ...
  availableSupporterList() {
    ...
    const [set, get] = args
    return get().userJoinRolePositionList().filter(userJoinRolePosition => {
      return !!userJoinRolePosition.id && userJoinRolePosition.id !== user.user_uuid && get().isPositionGroupInDirect(userJoinRolePosition.id)
    })
  },
  ...
  availableEmailForSubmitList() {
    ...
    const [set, get] = args
    return get().userJoinRolePositionList().filter(userJoinRolePosition => {
      return !!userJoinRolePosition.id && userJoinRolePosition.id != user.user_uuid && get().isPositionGroupDirect(userJoinRolePosition.id) && (get().isPositionLL(userJoinRolePosition.id) || get().isPositionTL(userJoinRolePosition.id) || get().isPositionMGR(userJoinRolePosition.id))
    })
  },
