...
export const RequestConfigStore = create<IBaseRequestConfigState & IRequestConfigState & IRequestFlowConfigState>((...args) => ({
  ...
  canEditRequest(action) {
    ...
    return user?.user_uuid === action.user_uuid && get().EditableTransitionNameList.includes(get().state[action?.current_state_id]?.name)
  },
  ...
  shouldPromptModal(transition) {
    ...
    return isShowActionNote || isShowMailingList || isShowSelectSupporter || isShowSelectConfirmation
  },

  isRequestProblemFinished(transition) {
    const [set, get] = args
    const { canCreateChangePointRequest } = _5M1ESettingStore.getState()
    return canCreateChangePointRequest() && get().GoToSubmitChangePointActionNameList.includes(get().action[transition?.action_id]?.name)
  },

  isShowActionNote(transition) {
    const [set, get] = args
    return get().PromptActionNoteActionNameList.includes(get().action[transition?.action_id]?.name)
  },

  isShowMailingList(transition) {
    const [set, get] = args
    return get().PromptMailActionNameList.includes(get().action[transition?.action_id]?.name)
  },

  isShowSelectSupporter(transition) {
    const [set, get] = args
    return get().PromptSelectSupporterActionNameList.includes(get().action[transition?.action_id]?.name)
  },

  isShowSelectConfirmation(transition) {
    const [set, get] = args
    return get().PromptSelectConfirmationActionNameList.includes(get().action[transition?.action_id]?.name)
  },
}))
