...
async def _check_flow(crud, config: dict, request_data: RequestCreateSubmitWithId, db: AsyncSession) -> None:
    ...
    for t in transition.values():
        if t['current_state_id'] == str(current_state_id) and t['next_state_id'] == str(next_state_id):
            ok = True
    if not ok:
        raise HTTPException(status_code=406, detail="This action is not allow. Please try to refresh to get latest data.")
...
async def _check_right(request_data: RequestCreateSubmit, db: AsyncSession) -> None:
    ...
    if user_uuid is None or action_id is None or action_user_uuid is None or data_value is None:
    ...
    print(f'CHECK Right: action_id = {action_id}, action_user = {action_user}, requester = {requester}')
    # submit, anyone can whoin position group direct
    if (action_id == 1 or action_id == 11) and action_user['position_group'] != 'Direct':
        raise HTTPException(status_code=403, detail="Submit action is forbidden")
    elif (action_id == 2 or action_id == 12) and not ((user_uuid == action_user_uuid) or (_i_am_higher(action_user['position_name'], requester['position_name']))):
        raise HTTPException(status_code=403, detail="Cancel action is forbidden")
    elif (action_id == 3 or action_id == 13) and ((user_uuid == action_user_uuid) or not (_i_am_higher(action_user['position_name'], requester['position_name']))):
        raise HTTPException(status_code=403, detail="Approve action is forbidden")
    elif (action_id == 4 or action_id == 14) and ((user_uuid == action_user_uuid) or not (_i_am_higher(action_user['position_name'], requester['position_name']))):
        raise HTTPException(status_code=403, detail="Reject action is forbidden")
    elif (action_id == 6 or action_id == 16) and ((user_uuid == action_user_uuid) or not _can_i_select_supporter(action_user['position_name'])):
        raise HTTPException(status_code=403, detail="Supporter selection is forbidden")
    elif (action_id == 7 or action_id == 17) and ((user_uuid == action_user_uuid) or not _can_i_select_supporter(action_user['position_name'])):
        raise HTTPException(status_code=403, detail="Supporter selection is forbidden")

async def _send_mail(crud, request_data: RequestCreateSubmit, request_id, db: AsyncSession) -> None:
    ...
    if user_uuid is None or action_id is None or email_list_data is None or action_user_uuid is None:
        raise HTTPException(status_code=422, detail="Request data is not completed")
    requester = await user_manager.get_by_user_uuid(user_uuid, db = db)

    request_no = request_response[-1]['request_problem_no'] if request_response[-1]['request_problem_no'] is not None else request_response[-1]['request_change_no']
    ...
    if (action_id == 1 or action_id == 11):
        ...
        if action_id == 1:
            ...
            with open('email_templates/email_template_request_to_check.txt', encoding='utf-8') as f:
                for line in f:
                    html += line.strip()
        else:
            subject = "You got a request to confirm about 5M1E change"
            with open('email_templates/email_template_request_to_check_change.txt', encoding='utf-8') as f:
                for line in f:
                    html += line.strip()
        ...
        for email in email_list:
            ...
            try:
                t = threading.Thread(target=send_mail, args=(email, subject, send_html, attachments))
                t.start()
        ...
        if detailId > 0:
            ...
            item_detail = detail or '-'
        html = ""
        with open('email_templates/email_template_self_cancel.txt', encoding='utf-8') as f:
            for line in f:
                html += line.strip()
        html = html.replace("{request_no}", request_no)
        html = html.replace("{requester_name}", requester_name)
        ...
        for email in email_list:
            ...
            send_html = html.replace("{checker_name}", checker_name)
            try:
                t = threading.Thread(target=send_mail, args = (email, "The 5M1E confirmation request has been cancelled", send_html, attachments))
                t.start()
                #send_mail(email, "You got a request to confirm about 5M1E problem", send_html, attachments)
            except Exception as e:
                print(f'Error when sending email: {e}')
        ...
        if detailId > 0:
            ...
            item_detail = detail or '-'
        html = ""
        with open('email_templates/email_template_cancel_request.txt', encoding='utf-8') as f:
            for line in f:
                html += line.strip()
        html = html.replace("{request_no}", request_no)
        html = html.replace("{requester_name}", requester_name)
        ...
        for email in email_list:
            send_html = html.replace("{checker_name}", checker_name)
            try:
                t = threading.Thread(target=send_mail, args=(email, "Your 5M1E confirmation request has been cancelled", send_html, attachments))
                t.start()
        ...
        if action_id == 4:
            ...
            with open('email_templates/email_template_reject_to_requester.txt', encoding='utf-8') as f:
                for line in f:
                    html += line.strip()
        else:
            subject = "Your 5M1E request has been rejected"
            with open('email_templates/email_template_reject_change_to_requester.txt', encoding='utf-8') as f:
                for line in f:
                    html += line.strip()
        html = html.replace("{request_no}", request_no)
        html = html.replace("{requester_name}", requester_name)
        ...
        if action_id == 3:
            ...
            with open('email_templates/email_template_inform_problem.txt', encoding='utf-8') as f:
                for line in f:
                    html += line.strip()
        else:
            subject = f"There is a change in {line_name}"
            with open('email_templates/email_template_inform_change.txt', encoding='utf-8') as f:
                for line in f:
                    html += line.strip()
        html = html.replace("{request_no}", request_no)
        html = html.replace("{requester_name}", requester_name)
        ...
        try:
            send_html = html
            t = threading.Thread(target=send_mail, args=(email_list, subject, send_html, attachments))
            t.start()
        ...
        if action_id == 6:
            ...
            with open('email_templates/email_template_request_supporter.txt', encoding='utf-8') as f:
                for line in f:
                    html += line.strip()
        else:
            subject = "There is a confirmation request about 5M1E change"
            with open('email_templates/email_template_request_confirmation.txt', encoding='utf-8') as f:
                for line in f:
                    html += line.strip()            
        html = html.replace("{request_no}", request_no)
        html = html.replace("{requester_name}", requester_name)
...
def request_routers(db: AsyncGenerator) -> APIRouter:
    ...
    def upload(files: List[UploadFile] = File(...)):
        ...
        for file in files:
            try:
                ...
                rd = ''.join(random.SystemRandom().choice(string.ascii_uppercase + string.digits) for _ in range(8))
                ...
                path_map[file.filename] = {'local_path': file_path, 'url': f'/static/{filename}'}
                file.file.close()
    ...
    async def submit_request(
        ...
        if config is None:
            raise HTTPException(status_code=422, detail="Request process name is unsupported")
        await _check_right(request_data, db)
        request_response = await crud.post_submit_request(request_data, db)
        await _send_mail(crud, request_data, list(request_response['request'].keys())[0], db)
        return request_response

    @router.post("/update", dependencies=[Depends(api_key_auth)])
    async def update_request(request_data: RequestCreateSubmitWithId, db: AsyncSession = Depends(db)):
        config = _get_config(request_data.request_process_name)
        if config is None:
            raise HTTPException(status_code=422, detail="Request process name is unsupported")
        await _check_flow(crud, config, request_data, db)
        await _check_right(request_data, db)
        request_response = await crud.post_update_request(request_data.request_id, request_data, db)
        await _send_mail(crud, request_data, list(request_response['request'].keys())[0], db)
        return request_response
    
    @router.post("/save", dependencies=[Depends(api_key_auth)])
    async def save_request(request_data: RequestCreateSubmitWithId, db: AsyncSession = Depends(db)):
        request_response = await crud.post_save_request(request_data.request_id, request_data, db)
        return request_response

    @router.get(
        "/get/allrequests", dependencies=[Depends(api_key_auth)]
    )
    async def get_all_requestse(db: AsyncSession = Depends(db)):
        requests = await crud.get_all_requests(db)
        return requests

    @router.get(
        "/get/allrequests_by_type", dependencies=[Depends(api_key_auth)]
    )
    ...
    @router.get(
        "/get/requests", dependencies=[Depends(api_key_auth)]
    )
