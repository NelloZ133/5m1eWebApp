...
class RequestCRUD:
    ...
    async def post_submit_request(
        ...
        if submit_data.request_process_name == '5M1E_change':
            stmt1 = st.create_problem_change(request_id, submit_data.data_value.get('problem_request_id', None))
    ...
    async def post_update_request(
        ...
        request = await self.get_request(request_id, db)
        if submit_data.request_process_name == '5M1E_change':
            stmt1 = st.update_problem_change(request_id, submit_data.data_value.get('problem_request_id', None))
            await db.execute(stmt1)
            await db.commit()
        return {"request": request}
    ...
    async def get_all_requests_by_type(
        ...
        stmt = st.get_all_requests_by_type_stmt(process_id, process_name)
        rs = toArrayWithKey(await db.execute(stmt))
        # rs = rearrangeRequestActionData(rs, "request_no")
        return rs
